...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Created trust model: {trust_model.name}'))
        ...
        self.stdout.write(self.style.SUCCESS(f'Created trust model environment: {yivi_tme.environment}'))
        ...
        email_attr, _ = CredentialAttribute.objects.get_or_create(
            ...
            name="pbdf.pbdf.email.email"
        )
        self.stdout.write(self.style.SUCCESS(f'Created credential attribute: {email_attr.name} with ID: {email_attr.id}'))
        
        domain_attr, _ = CredentialAttribute.objects.get_or_create(
            credential=email_credential,
            name="pbdf.pbdf.email.domain"
        )
        self.stdout.write(self.style.SUCCESS(f'Created credential attribute: {domain_attr.name} with ID: {domain_attr.id}'))
        
        mobile_attr, _ = CredentialAttribute.objects.get_or_create(
            credential=mobile_credential,
            name="pbdf.pbdf.mobilenumber.mobilenumber"
        )
        self.stdout.write(self.style.SUCCESS(f'Created credential attribute: {mobile_attr.name} with ID: {mobile_attr.id}'))
