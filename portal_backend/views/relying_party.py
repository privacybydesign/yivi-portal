...
class RelyingPartyListAPIView(APIView):
    ...
    def get(self, request, name: str, environment: str):
        ...
        relying_party = get_object_or_404(
            RelyingParty, yivi_tme__trust_model__name=name, yivi_tme__environment=environment)
...
class RelyingPartyRegisterAPIView(APIView):
    permission_classes = [permissions.IsAuthenticated,
                          BelongsToOrganization, IsMaintainer]
    ...
    def make_condiscon_from_attributes(self, attributes_data):
        ...
        for attr in attributes_data:
            credential_attribute = CredentialAttribute.objects.get(
                credential__credential_tag=attr['credential_attribute_tag'], name=attr['credential_attribute_name'])
            ...
            credential_attributes[credential_id].append(
                credential_attribute.name)
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            required=['hostname', 'trust_model_env', 'attributes',
                      'context_description_en', 'context_description_nl'],
            properties={
    ...
    def post(self, request, pk):
        ...
        condiscon = self.save_condiscon(
            request, attributes_data, relying_party)
    ...
    def patch(self, request, pk):
        ...
        if request.data.get("hostname") is not None:
            ...
            hostname_obj = get_object_or_404(
            ...
            if hostname_obj.hostname != hostname_text:
                ...
                hostname_obj.save()
                # add hostname dns challenge to response
        ...
        elif any(field in request.data for field in ["hostname", "context_description_en", "context_description_nl", "attributes", "trust_model_env"]):
            rp_status = Status.objects.get(relying_party=relying_party)
...
class RelyingPartyHostnameStatusAPIView(APIView):
    ...
    def get(self, request, slug):
        ...
        relying_party = get_object_or_404(
            RelyingParty, organization__slug=slug)
